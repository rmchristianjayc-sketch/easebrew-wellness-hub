...
const MOOD_LABELS = ["", "😢 Malungkot", "😕 Hindi Okay", "😐 Okay Lang", "🙂 Masaya", "😄 Napakasaya"];
const RELIEF_LABELS = ["", "😣 Walang Tulong", "😕 Konting Tulong", "😐 May Tulong", "🙂 Malaking Tulong", "😄 Lubos na Gaan"];
...
function getPainLabel(score: number) {
  ...
  if (score <= 4) return "🟡 Banayad";
  if (score <= 6) return "🟠 Katamtaman";
  if (score <= 8) return "🔴 Matindi";
  return "🔴 Matinding Matindi";
}
...
export default function TrackerPage() {
  ...
  return (
    <div style={{ maxWidth: 680, margin: "0 auto", background: CREAM, minHeight: "100vh", paddingBottom: 100 }}>
      ...
      <div style={{ background: G, padding: "24px 24px 20px", color: "#fff" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div style={{ textAlign: "center", background: "rgba(255,255,255,0.15)", borderRadius: 12, padding: "10px 16px" }}>
            <p style={{ fontSize: 28, fontWeight: 700, margin: 0, color: GOLD }}>Araw {currentDay}</p>
            <p style={{ fontSize: 12, margin: 0, opacity: 0.8 }}>sa 30</p>
          </div>
        </div>
      ...
      {view === "today" && (
        <div style={{ padding: "24px 20px" }}>
          <p style={{ fontSize: 15, color: MID, marginBottom: 20 }}>
            {new Date().toLocaleDateString("fil-PH", { weekday: "long", year: "numeric", month: "long", day: "numeric" })}
          </p>
          ...
          <div style={{ background: "#fff", borderRadius: 16, padding: 20, marginBottom: 16 }}>
            ...
            <p style={{ fontSize: 13, color: MID, margin: "0 0 16px 0" }}>1 = Halos wala, 10 = Matinding matindi</p>
          ...
          {/* Easebrew + Avocado Oil */}
          <div style={{ background: "#fff", borderRadius: 16, padding: 20, marginBottom: 16 }}>
            <h3 style={{ fontSize: 17, fontWeight: 700, color: DARK, margin: "0 0 16px 0" }}>☕ Wellness Checklist</h3>
            {[
              { key: "easebrew", label: "☕ Nainom ang Easebrew ngayon", val: todayEntry.easebrew },
          ...
          {/* Numbers */}
          <div style={{ background: "#fff", borderRadius: 16, padding: 20, marginBottom: 16 }}>
            <h3 style={{ fontSize: 17, fontWeight: 700, color: DARK, margin: "0 0 16px 0" }}>📊 Mga Bilang Ngayon</h3>
            {[
              { key: "tubig", label: "💧 Tubig (glasses)", max: 12, unit: "glasses" },
              { key: "exercise", label: "🏃 Exercise (minuto)", max: 120, unit: "mins" },
              { key: "tulog", label: "😴 Tulog (oras)", max: 12, unit: "hrs" },
            ].map(item => (
          ...
          <div style={{ background: "#fff", borderRadius: 16, padding: 20, marginBottom: 20 }}>
            <h3 style={{ fontSize: 17, fontWeight: 700, color: DARK, margin: "0 0 16px 0" }}>💊 Gamot at Notes</h3>
            <p style={{ fontSize: 14, color: MID, margin: "0 0 6px 0" }}>Gamot na nainom (optional)</p>
            <input
              type="text"
              placeholder="Hal: Biogesic, Alaxan..."
            ...
            <textarea
              placeholder="Ano ang nararamdaman mo ngayon? May napansin ka bang pagbabago?"
          ...
          >
            {saved ? "✅ Na-save na!" : "💾 I-save ang Entry Ngayon"}
          </button>
        </div>
      )}
      ...
      {view === "history" && (
        <div style={{ padding: "24px 20px" }}>
          <h2 style={{ fontSize: 20, fontWeight: 700, color: G, marginBottom: 16 }}>📅 Kasaysayan ng Pain</h2>
          {entries.length === 0 ? (
            <div style={{ textAlign: "center", padding: "40px 20px", color: MID }}>
              <p style={{ fontSize: 48 }}>📋</p>
              <p style={{ fontSize: 17 }}>Wala pang entries. Mag-log ng first day mo!</p>
            </div>
          ) : (
              ...
              <div key={i} style={{
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                  <p style={{ fontSize: 15, fontWeight: 700, color: DARK, margin: 0 }}>
                    {new Date(entry.date).toLocaleDateString("fil-PH", { month: "short", day: "numeric", year: "numeric" })}
                  </p>
                  <span style={{
      ...
      {view === "summary" && (
        <div style={{ padding: "24px 20px" }}>
          <h2 style={{ fontSize: 20, fontWeight: 700, color: G, marginBottom: 16 }}>📊 Iyong Progress</h2>

          {/* Stats */}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>
            {[
              { label: "Araw na Na-log", value: `${entries.length}/30`, icon: "📅" },
              { label: "Average Pain", value: avgPain, icon: "😣" },
              { label: "Easebrew Rate", value: `${easebrewRate}%`, icon: "☕" },
              { label: "Araw Remaining", value: `${Math.max(0, 30 - entries.length)}`, icon: "⏳" },
            ].map((stat, i) => (
              <div key={i} style={{
          ...
          {/* Weekly Summary */}
          <h3 style={{ fontSize: 17, fontWeight: 700, color: G, marginBottom: 12 }}>Linggo-Linggong Progress</h3>
          {[0, 1, 2, 3].map(week => {
              ...
              <div key={week} style={{
                ...
                <div>
                  ...
                  <p style={{ fontSize: 13, color: MID, margin: "2px 0 0 0" }}>
                    Araw {week * 7 + 1}–{Math.min((week + 1) * 7, 30)}
                  </p>
                </div>
                <div style={{ textAlign: "right" }}>
                  {avg ? (
                    <>
                      <p style={{ fontSize: 22, fontWeight: 700, color: getPainColor(Number(avg)), margin: 0 }}>
                        {avg}
                      </p>
                      <p style={{ fontSize: 12, color: MID, margin: 0 }}>avg pain</p>
                    </>
                  ) : (
                    <p style={{ fontSize: 14, color: "#ccc", margin: 0 }}>Hindi pa</p>
                  )}
                </div>
              </div>
            );
      ...
      {view === "journal" && (
        <div style={{ padding: "24px 20px" }}>
          ...
          <p style={{ fontSize: 15, color: MID, marginBottom: 20 }}>
            Isulat ang iyong nararamdaman, ang iyong laban, at ang iyong tagumpay.
          </p>
          {[
            { week: 1, title: "🌱 Linggo 1 — Simula ng Journey (Araw 1-7)", questions: ["Bakit ko sinimulang gamitin ang Easebrew?", "Ano ang pinaka-masakit na parte ng katawan ko?", "Ano ang gusto kong maramdaman pagkatapos ng 30 araw?"] },
            { week: 2, title: "💪 Linggo 2 — Unang Pagbabago (Araw 8-14)", questions: ["May napansin ba akong pagbabago sa nakaraang linggo?", "Paano ko nararamdaman ang Easebrew sa katawan ko?"] },
            { week: 3, title: "🌟 Linggo 3 — Progreso Ko (Araw 15-21)", questions: ["Kung ikukumpara ko ang sarili ko ngayon sa Linggo 1 — ano ang pagkakaiba?", "Ano ang pinaka-epektibong bahagi ng routine ko?"] },
            { week: 4, title: "🏆 Linggo 4 — Bagong Katawan (Araw 22-30)", questions: ["Ano ang pinakamalaking pagbabago na naramdaman ko?", "Irerekomenda ko ba ang Easebrew sa pamilya at kaibigan ko? Bakit?"] },
          ].map(section => (
            <div key={section.week} style={{
              ...
              <textarea
                placeholder="Isulat ang iyong nararamdaman dito..."
