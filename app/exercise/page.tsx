...
function useSessionGuard() {
  ...
  useEffect(() => {
    try {
      ...
      if (!raw) { router.replace("/verify?from=/exercise"); return; }
      const session = JSON.parse(raw);
      if (!session?.expires_at) { router.replace("/verify?from=/exercise"); return; }
      if (new Date(session.expires_at) < new Date()) {
        localStorage.removeItem("eb_session");
        router.replace("/verify?from=/exercise");
        return;
      }
      setReady(true);
    } catch { router.replace("/verify?from=/exercise"); }
  }, [router]);
  return ready;
}
// ─────────────────────────────────────────────────────────────
...
const EXERCISE_PROGRAM: Phase[] = [
  {
    ...
    days: [
      ...
      { day: 7, title: "Complete Rest", exercises: [{ name: "Full Rest Day", duration: "All day", rest: "N/A", instruction: "Pahinga ang katawan. Inumin ang Easebrew, mag-massage ng Avocado Oil, at matulog ng maaga." }] },
    ]
  },
  {
    ...
    days: [
      ...
      { day: 14, title: "Rest & Recovery", exercises: [{ name: "Rest Day", duration: "All day", rest: "N/A", instruction: "Pahinga ang katawan. Gamitin ang Avocado Oil para sa self-massage sa masakit na parte." }] },
    ]
  },
  {
...
export default function ExercisePage() {
  ...
  useEffect(() => {
    try {
      const savedDays = localStorage.getItem("eb_completed_days");
      const savedExercises = localStorage.getItem("eb_completed_exercises");
      if (savedDays) setCompletedDays(new Set(JSON.parse(savedDays)));
      if (savedExercises) setCompletedExercises(new Set(JSON.parse(savedExercises)));
    } catch {}
  }, []);
  ...
  return (
    <div style={{ maxWidth: 680, margin: "0 auto", background: CREAM, minHeight: "100vh", paddingBottom: 100 }}>
      ...
      <div style={{ background: G, padding: "44px 24px 36px", color: "#fff", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ display: "inline-block", background: GOLD, color: G, borderRadius: 20, padding: "6px 18px", fontSize: 13, fontWeight: 700, marginBottom: 14, letterSpacing: 1 }}>
          💪 30-DAY PROGRAM
        </div>
        <h1 style={{ fontSize: 34, fontWeight: 700, margin: "0 0 12px 0", color: "#fff", lineHeight: 1.2 }}>
          Home Exercise Guide
        </h1>
        <p style={{ fontSize: 18, opacity: 0.9, margin: "0 0 24px 0", lineHeight: 1.6 }}>
          Safe, low-impact exercises para sa joint pain.<br />Walang gym equipment needed!
        </p>
        {/* Progress Bar */}
      ...
      <div style={{ background: GOLD, padding: "14px 24px", display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <div>
          ...
          <p style={{ fontSize: 14, color: AMBER, margin: 0 }}>30 minuto bago simulan — para mas magaan ang katawan</p>
        </div>
      </div>
          ...
          {EXERCISE_PROGRAM.map((ph) => {
            ...
            return (
              <button key={ph.phase} onClick={() => { setSelectedPhase(ph.phase); setExpandedDay(null); }} style={{
                background: isActive ? c.bg : "#FFFFFB",
                border: `3px solid ${isActive ? c.border : "#C5B99A"}`,
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                  ...
                  <div>
                    <p style={{ fontSize: 13, fontWeight: 700, color: c.color, margin: "0 0 4px 0", textTransform: "uppercase", letterSpacing: 1 }}>
                      Phase {ph.phase}
                    </p>
                    <p style={{ fontSize: 20, fontWeight: 700, color: DARK, margin: "0 0 4px 0" }}>{ph.name}</p>
                    <p style={{ fontSize: 15, color: MID, margin: 0 }}>{ph.weeks} · {doneInPhase}/{ph.days.length} araw</p>
                  </div>
                </div>
      ...
      {/* ── DAYS LIST ── */}
      <div style={{ padding: "28px 24px 0" }}>
        <h2 style={{ fontSize: 22, fontWeight: 700, color: G, marginBottom: 16 }}>
          {currentPhase.name} — {currentPhase.weeks}
        </h2>
          ...
          {currentPhase.days.map((day) => {
            ...
            return (
              <div key={day.day} style={{
                background: "#FFFFFB",
                border: `2px solid ${isDone ? c.border : "#C5B99A"}`,
                borderRadius: 18,
                overflow: "hidden",
                boxShadow: isDone ? `0 2px 8px ${c.border}40` : "none",
              }}>
                {/* Day Header — big tap target */}
                <div
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                    ...
                    <div style={{
                      ...
                      background: isDone ? c.border : isRestDay ? "#F0EAE0" : G,
                      display: "flex", alignItems: "center", justifyContent: "center",
                      fontSize: isDone ? 22 : 18, fontWeight: 700,
                      color: isDone ? "#fff" : isRestDay ? MID : "#fff",
                      flexShrink: 0,
                    }}>
                  ...
                  <div style={{
                    ...
                    background: isOpen ? G : "#F0EAE0",
                    display: "flex", alignItems: "center", justifyContent: "center",
                    fontSize: 18, color: isOpen ? "#fff" : G,
                    flexShrink: 0,
                  }}>
                    {isOpen ? "▲" : "▼"}
                  </div>
                ...
                {isOpen && (
                  <div style={{ borderTop: "2px solid #E5DDD0" }}>
                      ...
                      return (
                        <div
                          ...
                          style={{
                            ...
                            cursor: "pointer",
                            background: exDone ? "#F4FAF0" : "transparent",
                          }}
                        >
                          <div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
                            ...
                            <div style={{
                              ...
                              border: `3px solid ${exDone ? G : "#C5B99A"}`,
                              background: exDone ? G : "transparent",
                              display: "flex", alignItems: "center", justifyContent: "center",
                              flexShrink: 0, marginTop: 2,
                            }}>
                              {exDone && <span style={{ color: "#fff", fontSize: 18, fontWeight: 700 }}>✓</span>}
                            </div>
                            <div style={{ flex: 1 }}>
                              {/* Exercise name */}
                              <p style={{
                              ...
                              <div style={{
                                ...
                                background: exDone ? "#D6EDCC" : "#E8F5E0",
                                borderRadius: 8, padding: "4px 12px",
                                marginBottom: 10,
                              }}>
                                <p style={{ fontSize: 15, color: G, fontWeight: 700, margin: 0 }}>
                              ...
                              {/* Instruction */}
                              <p style={{ fontSize: 17, color: DARK, margin: "0 0 8px 0", lineHeight: 1.6 }}>
                                {ex.instruction}
                              </p>
                              {/* Modification */}
                              {ex.modification && (
                                <div style={{
                                  ...
                                  <p style={{ fontSize: 15, color: AMBER, margin: 0, fontWeight: 600 }}>
                              ...
                              {/* Tap hint */}
                              {!exDone && (
                                <p style={{ fontSize: 13, color: MID, margin: "10px 0 0 0", opacity: 0.7 }}>
                                  I-tap para markahan bilang tapos ✓
                                </p>
                              )}
                            </div>
                          </div>
                    ...
                    {/* ── Mark Day Done Button — very large ── */}
                    <div style={{ padding: "20px 22px", background: "#FAFAF7" }}>
                      <button
                        onClick={() => toggleDayComplete(day.day)}
                        style={{
                          width: "100%",
                          background: isDone ? "#F5F0E8" : G,
                          color: isDone ? MID : "#fff",
                          border: isDone ? `2px solid #C5B99A` : "none",
                          borderRadius: 16,
                          padding: "20px",
                      ...
                      {!isDone && (
                        <p style={{ fontSize: 14, color: MID, textAlign: "center", margin: "10px 0 0 0", opacity: 0.8 }}>
                          I-tap pagkatapos ng lahat ng exercises
                        </p>
                      )}
                    </div>
                  </div>
      ...
      <div style={{ padding: "32px 24px 0" }}>
        <Link href="/" style={{
          display: "block", background: G, color: "#fff",
